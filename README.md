...
`content/cantonese/leon-lai/` 包含 4 套黎明的香港粤语访谈素材：
...
每套目录包含 MP3 原声、独立封面、香港粤语逐句字幕、简体中文翻译、VTT 时间轴和来源元数据。
字幕由本地语音模型生成，并将书面化识别词规范成实际粤语用词（例如“下班”改为
“放工”），再进行重复和异常片段过滤；中文译文以机器翻译方式生成并单独标注。运行以下命令可重建并校验四套素材：
